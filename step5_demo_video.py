...
def run_demo(confidence_threshold=0.6):
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            face_tensors.append(face_input)

        if faces:
            face_batch_cpu = torch.tensor(np.array(face_tensors), dtype=torch.float32)
            cpu_start = time.perf_counter()
            with torch.no_grad():
                cpu_out = model_cpu(face_batch_cpu)
            ...
            if model_gpu is not None:
                face_t_gpu = face_batch_cpu.cuda(non_blocking=True)
            ...
            cpu_fps_display = 0.9 * cpu_fps_display + 0.1 * cpu_fps
            if model_gpu is not None:
                gpu_fps_display = 0.9 * gpu_fps_display + 0.1 * gpu_fps

        for index, (x, y, w, h) in enumerate(faces):
            name = label_names[cpu_pred[index].item()] if cpu_conf[index].item() > confidence_threshold else "Unknown"
            gpu_name = label_names[gpu_pred[index].item()] if model_gpu is not None and gpu_conf[index].item() > confidence_threshold else "Unknown"
            cv2.rectangle(frame, (x, y), (x+w, y+h), (0, 255, 0), 2)
            cv2.putText(frame, f"CPU: {name} ({cpu_conf[index].item():.2f})", (x, y-30),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            cv2.putText(frame, f"GPU: {gpu_name}", (x, y-5),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 200, 0), 2)
        cv2.putText(frame, f"CPU CNN FPS: {cpu_fps_display:.1f}", (10, 30),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
        gpu_text = f"GPU CNN FPS: {gpu_fps_display:.1f}" if model_gpu is not None else "GPU: unavailable"
